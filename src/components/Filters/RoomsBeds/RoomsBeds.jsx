import "./RoomsBeds.css";
import { useFilter } from "../../../context";

const numberOfAmenities = ["Any", "1", "2", "3", "4", "5+"];

export const RoomsBeds = () => {

    const { filterDispatch, noOfBathrooms, noOfBedrooms, noOfBeds } = useFilter();

    const handleBedroomsClick = (number) => {
        filterDispatch({
            type: "BEDROOMS",
            payload: number
        })
    }

    const handleBedsClick = (number) => {
        filterDispatch({
            type: "BEDS",
            payload: number
        })
    }

    const handleBathroomsClick = (number) => {
        filterDispatch({
            type: "BATHROOMS",
            payload: number
        })
    }

    return (
        <div className="filter-container">
            <span className="filter-label">Rooms and Beds</span>
            <div className="cont-top1">
                <div className="cont-top2">
                    <span className="span-label">Bedrooms</span>
                    <span className="span-label">Beds</span>
                    <span className="span-label">Bathrooms</span>
                </div>
                <div className="amenities-1">
                    <div>
                        {
                            numberOfAmenities.map((number) => (
                                <span className={`span-label aminity-count item-1 cursor-pointer on-hover ${
                                    noOfBedrooms.toString() === number ? "selected" : ""
                                }`} key={number} onClick={() => handleBedroomsClick(number)}>
                                    {number}
                                </span>
                            ))
                        }
                    </div>

                    <div>

                        {
                            numberOfAmenities.map((number) => (
                                <span className={`span-label aminity-count item-1 cursor-pointer on-hover ${
                                    noOfBeds.toString() === number ? "selected" : ""
                                }`} key={number} onClick={() => handleBedsClick(number)}>
                                    {number}
                                </span>
                            ))
                        }
                    </div>
                    <div>
                        {
                            numberOfAmenities.map((number) => (
                                <span className={`span-label aminity-count item-1 cursor-pointer on-hover ${
                                    noOfBathrooms.toString() === number ? "selected" : ""
                                }`} key={number} onClick={() => handleBathroomsClick(number)}>
                                    {number}
                                </span>
                            ))
                        }
                    </div>
                </div>
            </div>
        </div>
    )
}